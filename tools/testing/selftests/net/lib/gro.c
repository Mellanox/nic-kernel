 ...
 *
 * psp_*:
 *  PSP transport mode encap, enabled with one or more --psp-assoc
 *  version,spi,key. The sender wraps and encrypts every frame in software;
 *  the receiving NIC is decrypts and decapsulates, so the receiver sees plain
 *  frames and all cases above apply unchanged. Both sides must be given the
 *  same --psp-assoc arguments: the receiver derives the expected MSS from the
 *  PSP encapsulation overhead (see calc_mss()).
 *   - psp_spi_diff: frames from two associations don't coalesce
 *   - psp_ver_diff: same, with two PSP versions (needs two assocs
 *                   of different versions)
 *   - psp_mixed:    a PSP frame doesn't coalesce with a clear one
 *
...
static bool psp_assoc_versions_differ(void)
{
	return psp_assocs[0].version != psp_assocs[1].version;
}
#else
static void psp_parse_assoc(char *arg)
{
	error(EXIT_NO_PSP_SUPPORT, 0,
	      "gro was built without OpenSSL, PSP is not available");
}
...
/* PSP packets don't coalesce across associations. Two RX associations
 * always have distinct SPIs, so the SPI mismatch alone terminates the
 * session; psp_ver_diff additionally checks that frames of two PSP
 * versions, decapsulated by separate steering rules, interleave on one
 * flow without merging. It cannot isolate the version comparison.
 */
static void send_psp_assoc_switch(int fd, struct sockaddr_ll *daddr)
{
	...
	if (!strcmp(testname, "psp_ver_diff") && !psp_assoc_versions_differ())
		error(1, 0, "%s needs two PSP versions", testname);
