...
class BandwidthValidator:
    ...
    def __init__(self):
        ...
        expected_total_gbps = 1.0
        tc3_expected_percent = 20.0
        tc4_expected_percent = 80.0

        # Pre-calculate all min/max values
        self.total_min_expected = self.min_expected(expected_total_gbps)
        self.total_max_expected = self.max_expected(expected_total_gbps)
        self.tc3_min_expected = self.min_expected(tc3_expected_percent)
        self.tc3_max_expected = self.max_expected(tc3_expected_percent)
        self.tc4_min_expected = self.min_expected(tc4_expected_percent)
        self.tc4_max_expected = self.max_expected(tc4_expected_percent)

    def min_expected(self, value):
        """Calculate the minimum acceptable value based on tolerance."""
        return value - (value * self.tolerance_percent / 100)

    def max_expected(self, value):
        """Calculate the maximum acceptable value based on tolerance."""
        return value + (value * self.tolerance_percent / 100)


def setup_vf(cfg, set_tc_mapping=True):
    """
    Set up a VF on the given network interface.
    ...
    try:
        ...
        raise KsftSkipEx(f"Failed to enable switchdev mode on {cfg.pci}: {exc}")
    try:
        cmd(f"echo 1 > /sys/class/net/{cfg.ifname}/device/sriov_numvfs")
        defer(cmd, f"echo 0 > /sys/class/net/{cfg.ifname}/device/sriov_numvfs")
    except Exception as exc:
        raise KsftSkipEx(f"Failed to enable SR-IOV on {cfg.ifname}: {exc}")
...
def setup_vlans_on_vf(vf_ifc):
    """
    Create two VLAN interfaces on the given VF, each mapped to a different TC.
    """
    vlan_configs = [
...
def get_vf_info(cfg):
    """
    Identifies the VF representor interface and devlink port index
...
def setup_bridge(cfg):
    """
    Creates and configures a Linux bridge, attaching both the uplink
    and VF representor interfaces to it.
    """
...
def setup_devlink_rate(cfg):
    """
    Configure devlink rate tx_max and traffic class bandwidth for the VF.
    """
    ...
    try:
        ...
        if exc.error == 95:  # EOPNOTSUPP
            raise KsftSkipEx("devlink rate or tc-bw not supported on the VF")
        raise KsftFailEx(f"rate_set failed on VF port {port_index}: {exc}")


def setup_remote_server(cfg):
    """
    Set up VLAN interfaces and start iperf3 servers on the remote side.
    """
...
def setup_test_environment(cfg, set_tc_mapping=True):
    """
    Set up the complete test environment including VF creation, VLANs,
    bridge configuration, devlink rate setup, and remote server.
    """
...
def run_iperf_client(server_ip, local_ip, barrier, min_expected_gbps=0.1):
    """
    Run a single iperf3 client instance, binding to the given local IP.
    Waits on a barrier to synchronize with other threads.
    """
    try:
        barrier.wait(timeout=10)
    except Exception as exc:
        raise KsftFailEx("iperf3 barrier wait timed")
...
def run_bandwidth_test():
    """
    Launch iperf3 client threads for each VLAN/TC pair and collect results.
    """
...
def calculate_bandwidth_percentages(results):
    """
    Calculate the percentage of total bandwidth received by TC3 and TC4.
    """
...
def print_bandwidth_results(bw_data, test_name):
    """
    Print bandwidth measurements and TC usage summary for a given test.
    """
...
def verify_total_bandwidth(bw_data, validator):
    """
    Ensure the total measured bandwidth falls within the acceptable tolerance.
    """
    total = bw_data['total_bw']

    if validator.total_min_expected <= total <= validator.total_max_expected:
        ksft_pr(f"Total bandwidth {total:.2f} Gbps within "
               f"[{validator.total_min_expected:.2f}, "
...
def check_bandwidth_distribution(bw_data, validator):
    """
    Check whether the measured TC3 and TC4 bandwidth percentages
    fall within their expected tolerance ranges.

    Returns:
        bool: True if both TC3 and TC4 percentages are within bounds.
    """
    tc3_valid = (validator.tc3_min_expected <= bw_data['tc3_percentage']
              <= validator.tc3_max_expected)
    tc4_valid = (validator.tc4_min_expected <= bw_data['tc4_percentage']
              <= validator.tc4_max_expected)
...
def test_no_tc_mapping_bandwidth(cfg):
    """Verifies bandwidth is not split 80/20 without traffic class mapping."""
...
def test_tc_mapping_bandwidth(cfg):
    """
    Verify that bandwidth is correctly split 80/20 between TC3 and TC4
