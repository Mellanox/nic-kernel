...
* **Owner structure**: Each PF embeds a ``devlink_shd_owner`` structure to track membership
...
3. **Get or create shared instance** using ``devlink_shd_get()``:
   ...
   * Adds PF to shared instance's device list via owner structure
...
2. **Call** ``devlink_shd_put()`` to release reference and remove PF from device list
...
A global mutex (``shd_mutex``) protects the shared instances list and individual shared
instance device lists during registration/deregistration.
