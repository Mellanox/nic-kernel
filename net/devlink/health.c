					...
					struct genl_info *info)
{
	...
	if (info->attrs[DEVLINK_ATTR_HEALTH_REPORTER_ERR_BURST_PERIOD]) {
		...
		if (!reporter->graceful_period && burst_period) {
			NL_SET_ERR_MSG_MOD(info->extack, "Cannot set error burst period without a grace period.");
			return -EINVAL;
		}
...
devlink_health_reporter_recovery_done(struct devlink_health_reporter *reporter)
{
	...
	if (!devlink_health_reporter_burst_period_active(reporter))
		/* When error burst period is set, last_recovery_ts marks
		 * the first recovery within the burst period, not necessarily the
		 * last one.
		 */
