...
static void nvme_unmap_data(struct nvme_dev *dev, struct request *req)
{
	...
	if (dma_can_use_iova(&iod->dma_state)) {
		/* SGL.ADDR is in the same place as PRP1 */
		dma_iova_destroy(dev->dev, &iod->dma_state,
				le64_to_cpu(iod->cmd.common.dptr.prp1),
				iod->total_len, rq_dma_dir(req));
	} else if (dma_need_unmap(dev->dev)) {
		if (iod->cmd.common.flags & NVME_CMD_SGL_METABUF)
		...
		struct request *req, struct nvme_rw_command *cmnd)
{
	...
	status = blk_rq_dma_map_iter_start(req, dev->dev, &iter,
			&iod->dma_state, &map);
	if (status)
		return status;
	...
	prp1_dma = map.addr;
	prp_len = min(length, NVME_CTRL_PAGE_SIZE -
			(map.addr & (NVME_CTRL_PAGE_SIZE - 1)));
	...
	if (!map.len) {
		if (!blk_rq_dma_map_iter_next(req, dev->dev, &iter,
				&iod->dma_state, &map, &status)) {
	...
	i = 0;
	for (;;) {
		prp_list[i++] = cpu_to_le64(map.addr);
		prp_len = min(length, NVME_CTRL_PAGE_SIZE);
		if (WARN_ON_ONCE(map.len < prp_len))
			goto bad_sgl;
		...
		struct request *req, struct nvme_rw_command *cmd)
{
	...
	if (entries == 1 || dma_can_use_iova(&iod->dma_state)) {
		nvme_pci_sgl_set_data(&cmd->dptr.sgl, &map);
		iod->total_len += map.len;
		return BLK_STS_OK;
	}
	...
	do {
		...
		nvme_pci_sgl_set_data(&sg_list[mapped++], &map);
		iod->total_len += map.len;
	} while (blk_rq_dma_map_iter_next(req, dev->dev, &iter, &iod->dma_state,
			&map, &status));
