	       ...
	       unsigned long arg)
{
	...
	case PTP_SYS_OFFSET_PRECISE2:
		return ptp_sys_offset_precise(ptp, argptr, ptp->info->getcrosststamp);

	case PTP_SYS_OFFSET_EXTENDED:
	case PTP_SYS_OFFSET_EXTENDED2:
		return ptp_sys_offset_extended(ptp, argptr, ptp->info->gettimex64);
	...
	case PTP_SYS_OFFSET_PRECISE_CYCLES:
		return ptp_sys_offset_precise(ptp, argptr, ptp->info->getcrosscycles);

	case PTP_SYS_OFFSET_EXTENDED_CYCLES:
		return ptp_sys_offset_extended(ptp, argptr, ptp->info->getcyclesx64);
	default:
		return -ENOTTY;
	}
}
