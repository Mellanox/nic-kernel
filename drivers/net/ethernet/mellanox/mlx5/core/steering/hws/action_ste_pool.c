...
static int
hws_action_ste_table_create_single_rtc(struct mlx5hws_context *ctx,
				       struct mlx5hws_action_ste_table *action_tbl,
				       enum mlx5hws_pool_optimize opt, size_t log_sz,
				       bool tx)
{
	...
	if (tx) {
		...
		rtc_attr.ste_base = mlx5hws_pool_get_base_mirror_id(action_tbl->pool);
		rtc_attr.stc_base = mlx5hws_pool_get_base_mirror_id(ctx->stc_pool);
		rtc_attr.log_size = opt == MLX5HWS_POOL_OPTIMIZE_ORIG ? 0 : log_sz;
		...
		rtc_attr.log_size = opt == MLX5HWS_POOL_OPTIMIZE_MIRROR ? 0 : log_sz;
				 ...
				 enum mlx5hws_pool_optimize opt, size_t log_sz)
{
	...
	err = hws_action_ste_table_create_single_rtc(ctx, action_tbl, opt, log_sz, false);
	if (err)
		return err;

	err = hws_action_ste_table_create_single_rtc(ctx, action_tbl, opt, log_sz, true);
	if (err) {
...
static void hws_action_ste_table_destroy_rtcs(struct mlx5hws_action_ste_table *action_tbl)
{
	mlx5hws_cmd_rtc_destroy(action_tbl->pool->ctx->mdev, action_tbl->rtc_1_id);
	mlx5hws_cmd_rtc_destroy(action_tbl->pool->ctx->mdev, action_tbl->rtc_0_id);
}

static int hws_action_ste_table_create_stc(struct mlx5hws_context *ctx,
					   struct mlx5hws_action_ste_table *action_tbl)
{
	...
	return mlx5hws_action_alloc_single_stc(ctx, &stc_attr,
					       MLX5HWS_TABLE_TYPE_FDB, &action_tbl->stc);
}
...
hws_action_ste_table_alloc(struct mlx5hws_action_ste_pool_element *parent_elem)
{
	...
	log_sz = min(parent_elem->log_sz
			     ? parent_elem->log_sz + MLX5HWS_ACTION_STE_TABLE_STEP_LOG_SZ
	...
	mlx5hws_dbg(ctx,
		    ...
		    action_tbl->rtc_0_id, action_tbl->rtc_1_id, action_tbl->stc.offset);
...
static void hws_action_ste_table_destroy(struct mlx5hws_action_ste_table *action_tbl)
{
	...
	mlx5hws_action_free_single_stc(ctx, MLX5HWS_TABLE_TYPE_FDB, &action_tbl->stc);
	hws_action_ste_table_destroy_rtcs(action_tbl);
	mlx5hws_pool_destroy(action_tbl->pool);
...
static void
hws_action_ste_pool_element_destroy(struct mlx5hws_action_ste_pool_element *elem)
{
				    ...
				    struct mlx5hws_action_ste_pool *pool)
{
	 ...
	 */
	for (opt = MLX5HWS_POOL_OPTIMIZE_NONE; opt < MLX5HWS_POOL_OPTIMIZE_MAX; opt++) {
		err = hws_action_ste_pool_element_init(ctx, &pool->elems[opt], opt);
		if (err)
			goto destroy_elems;
...
static void hws_action_ste_pool_destroy(struct mlx5hws_action_ste_pool *pool)
{
	...
	for (opt = MLX5HWS_POOL_OPTIMIZE_MAX - 1; opt >= MLX5HWS_POOL_OPTIMIZE_NONE; opt--)
		hws_action_ste_pool_element_destroy(&pool->elems[opt]);
}

static void
hws_action_ste_pool_element_collect_stale(struct mlx5hws_action_ste_pool_element *elem,
...
static void hws_action_ste_pool_cleanup(struct work_struct *work)
{
	...
	ctx = container_of(work, struct mlx5hws_context, action_ste_cleanup.work);
	...
	for (i = 0; i < ctx->queues; i++) {
		...
		for (opt = MLX5HWS_POOL_OPTIMIZE_NONE;
			...
			hws_action_ste_pool_element_collect_stale(&p->elems[opt], &cleanup);
		mutex_unlock(&p->lock);
	}
...
int mlx5hws_action_ste_pool_init(struct mlx5hws_context *ctx)
{
	...
	INIT_DELAYED_WORK(&ctx->action_ste_cleanup, hws_action_ste_pool_cleanup);
	schedule_delayed_work(&ctx->action_ste_cleanup,
			      secs_to_jiffies(MLX5HWS_ACTION_STE_POOL_CLEANUP_SECONDS));
				   ...
				   struct mlx5hws_action_ste_chunk *chunk)
{
	...
	pr_debug("Allocating action STEs skip_rx %d skip_tx %d order %d\n",
