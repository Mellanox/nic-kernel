...
mlx5_fs_get_cached_hws_data(struct xarray *cache_xa, unsigned long index)
{
	...
	if (!fs_hws_data) {
		fs_hws_data = kzalloc(sizeof(*fs_hws_data), GFP_KERNEL);
		if (!fs_hws_data) {
			xa_unlock(cache_xa);
			return NULL;
		}
		...
		err = __xa_insert(cache_xa, index, fs_hws_data, GFP_KERNEL);
		if (err) {
			kfree(fs_hws_data);
			xa_unlock(cache_xa);
			return NULL;
...
static void mlx5_fs_destroy_fs_actions(struct mlx5_flow_root_namespace *ns,
	...
	if (fte_action->action & MLX5_FLOW_CONTEXT_ACTION_FWD_DEST) {
		list_for_each_entry(dst, &fte->node.children, node.list) {
			...
			case MLX5_FLOW_DESTINATION_TYPE_FLOW_SAMPLER:
				dest_action =
					mlx5_fs_get_dest_action_sampler(fs_ctx, dst);
				fs_actions[num_fs_actions].action = dest_action;
