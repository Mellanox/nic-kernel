...
static int hws_debug_dump_context_stc(struct seq_file *f, struct mlx5hws_context *ctx)
{
	...
	if (stc_pool->resource) {
		ret = hws_debug_dump_context_stc_resource(f, ctx, stc_pool->resource);
		if (ret)
			return ret;
	}

	if (stc_pool->mirror_resource) {
		ret = hws_debug_dump_context_stc_resource(f, ctx, stc_pool->mirror_resource);
		if (ret)
			return ret;
	}
...
static void hws_debug_dump_action_ste_table(struct seq_file *f,
					   ...
					   struct mlx5hws_action_ste_pool *pool)
{
	...
	for (opt = MLX5HWS_POOL_OPTIMIZE_NONE; opt < MLX5HWS_POOL_OPTIMIZE_MAX; opt++) {
		list_for_each_entry(action_tbl, &pool->elems[opt].available, list_node) {
