...
static int hws_matcher_create_rtc(struct mlx5hws_matcher *matcher)
{
	...
	ret = mlx5hws_cmd_rtc_create(ctx->mdev, &rtc_attr, &matcher->match_ste.rtc_0_id);
	if (ret) {
	...
	if (tbl->type == MLX5HWS_TABLE_TYPE_FDB) {
		obj_id = mlx5hws_pool_get_base_mirror_id(matcher->match_ste.pool);
		rtc_attr.ste_base = obj_id;
...
static int hws_matcher_grow_at_array(struct mlx5hws_matcher *matcher)
{
	...
	p = krealloc(matcher->at, matcher->size_of_at_array * sizeof(*matcher->at),
			  ...
			  u8 num_of_at)
{
	...
	matcher->size_of_at_array = num_of_at + matcher->attr.max_num_of_at_attach;
