					 ...
					 bool skip_tx)
{
	...
	rule->action_ste.ste.order = ilog2(roundup_pow_of_two(matcher->action_ste.max_stes));
	return mlx5hws_action_ste_chunk_alloc(&ctx->action_ste_pool[queue_id],
					      skip_rx, skip_tx, &rule->action_ste);
}
				 ...
				 bool is_update)
{
	...
	if (!is_update) {
		...
		/* Save the old action STE info so we can free it after writing new
		 * action STEs and a corresponding match STE.
		 */
			       ...
			       struct mlx5hws_rule_action rule_actions[])
{
	...
	if (action_stes) {
		...
		ret = mlx5hws_rule_alloc_action_ste(rule, attr->queue_id,
						    !!ste_attr.rtc_0, !!ste_attr.rtc_1);
		if (ret) {
			mlx5hws_err(ctx, "Failed to allocate action memory %d", ret);
			mlx5hws_send_abort_new_dep_wqe(queue);
			return ret;
		}
		apply.jump_to_action_stc = rule->action_ste.action_tbl->stc.offset;
		/* Skip RX/TX based on the dep_wqe init */
		ste_attr.rtc_0 = dep_wqe->rtc_0 ? rule->action_ste.action_tbl->rtc_0_id : 0;
		ste_attr.rtc_1 = dep_wqe->rtc_1 ? rule->action_ste.action_tbl->rtc_1_id : 0;
		/* Action STEs are written to a specific index last to first */
		ste_attr.direct_index = rule->action_ste.ste.offset + action_stes;
