					     ...
					     struct mlx5e_ipsec_rx *rx)
{
	...
	MLX5_SET_TO_ONES(fte_match_param, match_criteria, misc_parameters_2.ipsec_syndrome);
	MLX5_SET_TO_ONES(fte_match_param, match_criteria, misc_parameters_2.metadata_reg_c_4);

	MLX5_SET(create_flow_group_in, flow_group_in, start_flow_index, ft->max_fte - 3);
	MLX5_SET(create_flow_group_in, flow_group_in, end_flow_index, ft->max_fte - 2);
				       ...
				       struct mlx5_flow_destination *dest)
{
	...
	rule = ipsec_rx_status_pass_create(ipsec, rx, dest, MLX5_IPSEC_ASO_SW_CRYPTO_OFFLOAD);
	if (IS_ERR(rule)) {
		err = PTR_ERR(rule);
	...
	pol_dest[1] = dest[1];
	rule = ipsec_rx_status_pass_create(ipsec, rx, pol_dest, MLX5_IPSEC_ASO_OK);
	if (IS_ERR(rule)) {
		err = PTR_ERR(rule);
				      ...
				      struct mlx5_flow_destination *miss_dest)
{
	if (rx == ipsec->rx_esw)
		...
		*miss_dest = mlx5_ttc_get_default_dest(attr->ttc, family2tt(attr->family));
}

static void ipsec_rx_default_dest_get(struct mlx5e_ipsec *ipsec, struct mlx5e_ipsec_rx *rx,
				      struct mlx5_flow_destination *dest)
{
	dest->type = MLX5_FLOW_DESTINATION_TYPE_FLOW_TABLE;
				  ...
				  struct mlx5_flow_destination *dest)
{
	...
	if (mlx5_ipsec_device_caps(mdev) & MLX5_IPSEC_CAP_PRIO) {
		...
		ft = ipsec_ft_create(attr->ns, attr->pol_level, attr->prio, 1, 2, 0);
		if (IS_ERR(ft)) {
			err = PTR_ERR(ft);
			goto err_out;
		}
		...
		err = ipsec_miss_create(mdev, rx->ft.pol, &rx->pol, &default_dest);
		if (err)
			mlx5_destroy_flow_table(rx->ft.pol);
	}
				       ...
				       struct mlx5e_ipsec_rx_create_attr *attr)
{
	...
	dest.type = MLX5_FLOW_DESTINATION_TYPE_COUNTER;
	dest.counter = fc;
	flow_act.action = MLX5_FLOW_CONTEXT_ACTION_COUNT | MLX5_FLOW_CONTEXT_ACTION_DROP;
			       ...
			       struct mlx5_flow_act *flow_act)
{
	...
	case XFRM_DEV_OFFLOAD_IN:
		...
		if (type == XFRM_DEV_OFFLOAD_CRYPTO) {
			...
			MLX5_SET(set_action_in, action[2], data, MLX5_IPSEC_ASO_SW_CRYPTO_OFFLOAD);
...
void mlx5e_accel_ipsec_fs_read_stats(struct mlx5e_priv *priv, void *ipsec_stats)
{
	...
	if (ipsec->is_uplink_rep) {
		...
		if (ipsec->rx_esw->sa_sel.fc &&
		    !mlx5_fc_query(mdev, ipsec->rx_esw->sa_sel.fc, &packets, &bytes))
