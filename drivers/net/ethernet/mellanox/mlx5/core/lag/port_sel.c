					  ...
					  u8 *ports)
{
	...
	mlx5_ldev_for_each(i, 0, ldev) {
		for (j = 0; j < ldev->buckets; j++) {
			...
			/* affinity is 1-indexed device index, use reverse lookup */
			dest.vport.vhca_id = MLX5_CAP_GEN(mlx5_lag_pf_by_dev_idx(ldev, affinity - 1)->dev,
							  vhca_id);
						   ...
						   u8 *ports)
{
	...
	mlx5_ldev_for_each(i, 0, ldev) {
		for (j = 0; j < ldev->buckets; j++) {
			...
			/* ports[] contains 1-indexed device indices, use reverse lookup */
			dest.vport.vhca_id = MLX5_CAP_GEN(mlx5_lag_pf_by_dev_idx(ldev, ports[idx] - 1)->dev,
							  vhca_id);
