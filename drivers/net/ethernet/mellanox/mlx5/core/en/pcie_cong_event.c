...
static bool mlx5e_thresh_check_val(u16 val)
{
	return val > 0 && val <= MLX5E_PCIE_CONG_THRESH_MAX;
}

static bool
mlx5e_thresh_config_validate(const struct mlx5e_pcie_cong_thresh *config)
{
	if (!mlx5e_thresh_check_val(config->inbound_high))
		return false;
	if (!mlx5e_thresh_check_val(config->inbound_low))
		return false;
	if (!mlx5e_thresh_check_val(config->outbound_high))
		return false;
	if (!mlx5e_thresh_check_val(config->outbound_low))
		return false;
				   ...
				   size_t count)
{
	...
	if (ret != MLX5E_PCIE_CONG_THRESH_SYSFS_VALUES) {
		mlx5_core_err(priv->mdev, "Invalid format for PCIe congestion threshold configuration. Expected %d, got %d.\n",
			      MLX5E_PCIE_CONG_THRESH_SYSFS_VALUES, 4);
		return -EINVAL;
	}

	if (!mlx5e_thresh_config_validate(&config)) {
