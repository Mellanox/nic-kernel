				 ...
				 enum dma_data_direction dir)
{
	...
	if (dma_can_use_iova(state)) {
		dma_iova_destroy(mdev->device, state, state->addr,
				 state->mapped_size, dir);
	} else {
			      ...
			      enum dma_data_direction dir)
{
	...
	if (dma_can_use_iova(state)) {
		addr = dma_iova_alloc(mdev->device, state, 0,
				npages * PAGE_SIZE);
		if (dma_mapping_error(mdev->device, addr))
			return -ENOMEM;

		for (i = 0; i < npages; i++) {
			err = dma_iova_link_next(mdev->device, state,
						 page_to_phys(page_list[i]),
						 PAGE_SIZE, dir, 0);
			if (err)
				goto error;
		}
			 ...
			 enum dma_data_direction dma_dir)
{
	...
	dma_iova_init(mdev->device, &buf->state);
	buf->dma_dir = dma_dir;
	buf->migf = migf;
	if (npages) {
